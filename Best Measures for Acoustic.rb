...
# Hello, I'm Victor Leao. Please, don't forget to give me the credits if you use this code.
# The aim of this code is to help you find the best measurements for your recording studio or any listening room. I hope you enjoy! =D

print "BEST MEASURES FOR ACOUSTIC
ver. 1.5 alpha

Insert the following values separated by commas:
a) Maximum value of lower measurement;
b) Maximum value of middle measurement;
c) Maximum value of higher measurement;
d) Average indoor temperature.
(E.g.: 6.98, 8, 13.2, 20)\n"
...
puts i
i=i.split(",")
max=[((i[0].to_f)*100).round, ((i[1].to_f)*100).round, ((i[2].to_f)*100).round].sort #maximum
max.freeze
t=i[3].to_f # temperature

# VARIABLES
best=[] # best measures
bs=[] # partial best score
hvol=nil # higher volume
...
for xl in (max[2]-start..max[2])
next if xl%mm!=0
next if xl<200
for xw in (max[1]-start..max[1])
next if xw%mm!=0
next if xw<200
for xh in (max[0]-start..max[0])
next if xh%mm!=0
next if xh<200

# BASIC
l, w, h = xl/100.00, xw/100.00, xh/100.00
values=[h, w, l].sort # current measurements
surf=((l*w)+(w*h)+(l*h))*2 # total surface
vol=l*w*h # volume
...
if bs[0]==nil
bs=[score, modals, ratios, reflecs, music]
hvol=vol
best=[l, w, h].sort
end
if score>bs[0] || (score==bs[0] && vol>hvol)
bs=[score, modals, ratios, reflecs, music]
hvol=vol
best=[l, w, h].sort
end
...
Musical: #{bs[4].round}
Ratio: #{bs[2].round}
Reflections: #{bs[3].round}
\nFINAL SCORE: #{bs[0].round}"

=begin
sequência de modos: Fibonacci, Lucas e geométrica
organizar em def e classes
=end
